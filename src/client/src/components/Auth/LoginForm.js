...
import {loginPatient, loginHelper, loginDoctor} from '../../api/index'
...
const SignupForm = ({ isLogin, setIsLogin, selectedCSC, setSelectedCSC, userType }) => {
  ...
  const loginIntern = async (sendData) => {
    try {
      ...
      if(userType==="DOCTOR")
        response = await loginDoctor(sendData);

      else if(userType==="PATIENT")
        response = await loginPatient(sendData);

      else if(userType==="HELPER")
        response = await loginHelper(sendData);
  ...
  return (
    <div class="w-full sm:mx-auto mx-0">
