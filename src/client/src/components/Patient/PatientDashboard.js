...
import { getAppointmentsByPatient, getEmergenciesByPatient, getPatientById } from '../../api'
  ...
  return (
    <div className='col-span-7 md:col-span-5'>
        ...
        <div className='mt-10 grid md:grid-cols-3 gap-10'>
                <div onClick={navigateToBookAppointment} className='p-5 place-content-center rounded-xl shadow-2xl hover:shadow-sm cursor-pointer'>
                    <img src={require('./static/video-conference.png')} className='mx-auto h-64' />
                ...
                <div onClick={navigateToCreateSOS} className='p-5 place-content-center shadow-2xl hover:shadow-sm cursor-pointer'>
                    <img src={require('./static/ambulance.png')} className='mx-auto h-64'/>
                ...
                <div onClick={navigateToOrderPrescription} className='p-5 shadow-2xl place-content-center hover:shadow-sm cursor-pointer'>
                    <img src={require('./static/medicine.png')} className='mx-auto h-64' />
                ...
                <div className="w-full flex justify-center m-10">
                    ...
                    <button className="px-4 py-2 bg-blue-300 text-3xl m-1 font-semibold rounded focus:bg-green-500 hover:bg-blue-700" onClick={()=>setCurrentAppointments(2)}>
                ...
                {currentAppointments===0 && <div className='mt-10'>
                {
                    emergencies.map((appointment)=>{
                        ...
                        return (
                            <div>
                                {appointment.status!=="resolved" && <div onClick={()=>{
            ...
            {currentAppointments===1 &&<div className='mt-10'>
                {
                    appointments.filter(x=>x.status=='scheduled').map((appointment)=><div onClick={()=>navigateToAppointment(appointment._id)} className='mt-5 p-5 shadow-2xl cursor-pointer'>
                        <p className='text-2xl'>{appointment.description}</p>
                        <p className='text-xl'>{appointment.dateCreated}</p>
                    </div>)
                }
            ...
            {currentAppointments===2 && <div className='mt-10'>
                {
                    appointments.filter(x=>x.status=='created').map((appointment)=><div onClick={()=>navigateToAppointment(appointment._id)} className='mt-5 p-5 shadow-2xl cursor-pointer'>
                        <p className='text-2xl'>{appointment.description}</p>
                        <p className='text-xl'>{appointment.dateCreated}</p>
                    </div>)
                }
