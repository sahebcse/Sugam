...
import {getBookedPrescriptions, getResolvedPrescriptions} from '../../actions/User'
...
export default function HealthWorkerDashboard() {
  ...
  useEffect(()=>{
      getBookedPrescriptions(setBookedPrescriptions,{id:user._id, status:"scheduled"});
      getResolvedPrescriptions(setResolvedPrescriptions,{id:user._id, status:"resolved"});
  },[])
  ...
  const navigateToWorkerRespondSos=()=>
  [
      navigate('/worker_respond_to_sos')
  ]
  return (
    <div className='col-span-5'>
      ...
      <div className='mt-10 grid md:grid-cols-3 gap-10'>
        <div onClick={navigateToConfirmPrescription} className='p-5 place-content-center rounded-xl shadow-2xl hover:shadow-sm cursor-pointer'>
    ...
    <div className="w-full flex justify-center m-10">
        <button className="px-4 py-2 bg-blue-300 text-3xl m-1 font-semibold rounded focus:bg-green-500 hover:bg-blue-700" onClick={()=>setCurrentPrescriptions(0)}>
            Booked Prescriptions
        </button>
        <button className="px-4 py-2 bg-blue-300 text-3xl m-1 font-semibold rounded focus:bg-green-500 hover:bg-blue-700" onClick={()=>setCurrentPrescriptions(1)}>
            Resolved Prescriptions
        </button>
    </div>

    {currentPrescriptions===0 && <div className='mt-10'>
        {
    ...
    {currentPrescriptions===1 && <div className='mt-10'>
          {
